        ...
        <header  className='bg-slate-200 shadow-md '>
            <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
                ...
                <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
                    <span className='text-slate-500'>Binay</span>
                ...
                <ul className="flex gap-4">
                    ...
                    <Link to='/profile'> {currentUser ? (<img className="rounded-full h-7 w-7 object-cover"
                     src={currentUser.avatar} alt={currentUser.username}/>):(<li className=" sm:inline text-slate-700 cursor-pointer">Sign in</li>)}</Link>
                </ul>
            </div>
        </header>
    )
